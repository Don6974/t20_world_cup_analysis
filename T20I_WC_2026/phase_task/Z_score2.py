...
# =====================================================
# STAGE 6: BOWLING – PHASE AWARE MODEL
# =====================================================
...
# =====================================================
# STAGE 6A: POWERPLAY ISOLATION
# =====================================================

pp_df = df[(df["phase"]=="Powerplay") & (df["legal"])]

pp_grp = pp_df.groupby("bowler").agg(
    pp_runs=("total_runs","sum"),
    pp_balls=("legal","sum"),
    pp_wkts=("is_wicket","sum")
)

pp_grp["pp_overs"] = pp_grp["pp_balls"] / 6
pp_grp = pp_grp[pp_grp["pp_overs"] >= 2]

pp_grp["pp_econ"] = pp_grp["pp_runs"] / pp_grp["pp_overs"]
pp_grp["pp_wkt_rate"] = pp_grp["pp_wkts"] / pp_grp["pp_overs"]

pp_grp["PP_Index"] = (
    0.6*z_score(pp_grp["pp_wkt_rate"]) +
    0.4*z_score(-pp_grp["pp_econ"])
)

bowl = bowl.join(pp_grp["PP_Index"]).fillna(0)

# =====================================================
# STAGE 6B: DEATH ISOLATION
# =====================================================
...
# =====================================================
# CLASSIFY BOWLING ROLE
# =====================================================

bowl["bowling_type"] = np.where(
    bowl.index.isin(SPINNER_LIST),
    "Spinner",
    "Pace"
)

bowl["bowling_role"] = "Middle"

bowl.loc[bowl["Death_Index"] > bowl["Impact_Index"], "bowling_role"] = "Death"

bowl.loc[
    (bowl["bowling_type"]=="Spinner") &
    (bowl["Impact_Index"] >= bowl["Death_Index"]),
    "bowling_role"
] = "Spinner"

bowl.loc[
    (bowl["PP_Index"] > bowl["Impact_Index"]) &
    (bowl["bowling_type"]=="Pace"),
    "bowling_role"
] = "Powerplay"

# =====================================================
# STAGE 6C: PHASE EXPOSURE BASED ROLE
# =====================================================

phase_usage = df[df["legal"]].groupby(["bowler","phase"]).size().unstack(fill_value=0)

# Ensure correct column names exist
for col in ["PP", "Early_Middle", "Late_Middle", "Death"]:
    if col not in phase_usage.columns:
        phase_usage[col] = 0

# Create merged columns
phase_usage["Powerplay"] = phase_usage["PP"]
phase_usage["Middle"] = (
    phase_usage["Early_Middle"] +
    phase_usage["Late_Middle"]
)

phase_usage = phase_usage.div(phase_usage.sum(axis=1), axis=0)

phase_usage = phase_usage[["Powerplay", "Middle", "Death"]]

bowl = bowl.join(phase_usage).fillna(0)

# Role classification with threshold
bowl["bowling_role"] = bowl[["Powerplay","Middle","Death"]].idxmax(axis=1)
# =====================================================
# STAGE 6C: MIDDLE OVERS ISOLATION
# =====================================================

middle_df = df[
    (df["phase"].isin(["EarlyMiddle","LateMiddle"])) &
    (df["legal"])
]

middle_grp = middle_df.groupby("bowler").agg(
    middle_runs=("total_runs","sum"),
    middle_balls=("legal","sum"),
    middle_wkts=("is_wicket","sum"),
    middle_dots=("is_dot","sum")
)

middle_grp["middle_overs"] = middle_grp["middle_balls"] / 6
middle_grp = middle_grp[middle_grp["middle_overs"] >= 2]

middle_grp["middle_econ"] = (
    middle_grp["middle_runs"] / middle_grp["middle_overs"]
)

middle_grp["middle_wkt_rate"] = (
    middle_grp["middle_wkts"] / middle_grp["middle_overs"]
)

middle_grp["middle_dot_pct"] = (
    middle_grp["middle_dots"] / middle_grp["middle_balls"]
)

middle_grp["Middle_Index"] = (
    0.4*z_score(middle_grp["middle_wkt_rate"]) +
    0.3*z_score(-middle_grp["middle_econ"]) +
    0.3*z_score(middle_grp["middle_dot_pct"])
)

bowl = bowl.join(middle_grp["Middle_Index"]).fillna(0)

print("Bowling role distribution:")
print(bowl["bowling_role"].value_counts())
print(bowl[["Powerplay","Middle","Death"]].head(20))

# =====================================================
# STAGE 7: STRUCTURED FINAL XI
# =====================================================
...
if len(ar_pool) >= 2:
    ...
    ar_df["cons"] = bat.loc[ar_pool]["consistency"]

    ar_df["AR_Index"] = (
        0.4*ar_df["bat"] +
        0.4*ar_df["bowl"] +
        0.2*z_score(ar_df["cons"])
    )

    pick(ar_df.sort_values("AR_Index",ascending=False), 2, "All-Rounder")

elif len(ar_pool) == 1:
    pick(pd.DataFrame(index=ar_pool), 1, "All-Rounder")
# ---- POWERPLAY ----
pick(
    bowl[bowl["bowling_role"]=="Powerplay"]
    .sort_values("PP_Index",ascending=False),
    2,
    "Powerplay"
)
...
# ---- MIDDLE ----
pick(
    bowl[bowl["bowling_role"]=="Middle"]
    .sort_values("Middle_Index",ascending=False),
    1,
    "Middle Bowler"
)
...
for player in selected:
    print(f"{player}  →  {role_map.get(player)}")
